...
State vector: $\vec{x} = [E_i(k), Q_i(k), T_i(k)]^\top \, \forall \, i \in [1,2], \, k\in[0,N]$

Decision vector: $\vec{u} = [c_i(k), d_i(k), u_i(k)]^\top \, \forall \, i \in [1,2], \, k \in [0, N-1]$
...
* $Q_i(k+1) = Q_i(k) - \alpha\phi\big(c_i(k)+d_i(k)\big)\Delta t$
	* where $\phi = 1 + \kappa \max \{ 0, T(k) - T_\text{ref}\}$
